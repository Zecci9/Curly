...
const name=computed(()=>decodeURIComponent(String(route.params.name))),filtered=computed(()=>posts.value.filter(p=>(p.tags||[]).includes(name.value)))
</script><template><section class="section"><div class="page-shell"><span class="eyebrow">Tag</span><h1 class="page-title">#{{name}}</h1><div class="grid"><PostCard v-for="p in filtered" :key="p.id" :post="p"/></div></div></section></template>
